...
class Polls_View
{
	public function render_recipients_metabox(Recipients_List $recipients_list, $post_id)
	{
		...
		<div class="wrap">
			...
			echo '<a href="'.$add_default_url.'" class="meta-menu-head">'.__('Add default recipients', LANG_DOMAIN).'</a>';
			$export_url = add_query_arg('action', ACTION_EXPORT);
			$export_url = wp_nonce_url($export_url, ACTION_EXPORT);
			echo ' <a href="'.$export_url.'" class="meta-menu-head">'.__('Export', LANG_DOMAIN).'</a>';
